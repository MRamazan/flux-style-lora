...
The cell installs the package, initializes the pipeline, installs or repairs the isolated runtime, verifies source compatibility, and prepares the training assets, including the FLUX model snapshot. It uses no Hugging Face token; every repository is downloaded anonymously.
...
The cell prepares the FLUX evaluation model, evaluates the run, displays all comparison grids, packages a portable evaluation bundle, and optionally downloads it.

## Evaluation-only notebook

`notebooks/flux_style_lora_evaluation_colab.ipynb` has exactly three cells.

### Cell 1: Setup

Installs the package from an explicit revision, initializes `StyleLoraPipeline`, and
verifies the isolated runtime. It does not download training assets and uses no Hugging
Face token. The FLUX model snapshot is prepared in cell three.
