...
Contains validated style-only configuration dataclasses. It contains no concept selector.
...
Resolves and anonymously downloads the FLUX Diffusers snapshot, verifies its component folders, and records repository provenance.

### `isolated/`

Self-contained scripts executed by the isolated AI Toolkit interpreter for GPU-bound work: source preflight, the non-destructive FLUX runtime helper, and the base, checkpoint, and scale evaluation passes. These modules are never imported by the outer package.
...
Contains non-destructive FLUX adapter loading and scale control used by evaluation subprocesses.
...
The default workspace is `/content/flux_style_lora`.
...
The outer package remains lightweight. AI Toolkit, PyTorch, FLUX integration dependencies, and model execution run inside the isolated environment created under the workspace. The package communicates with heavy runtime tasks through explicit subprocess commands, environment variables, files, structured JSON output, and logs.
