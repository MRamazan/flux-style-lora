...
The result must be style-only. Remove the public and internal `concept_type` setting instead of hardcoding it to `style`. Do not leave dead branches for style, object, product, or generic concept training. Keep only style-relevant behavior and neutral internal naming where required by third-party APIs.
