...
Refactor the reference notebook into a production-quality, style-only Python package and a compact Google Colab notebook.
...
- Style LoRA training is the only supported workflow.
...
- Train and evaluate with the same FLUX base model snapshot; there is no separate turbo variant.
