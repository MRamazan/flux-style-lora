...
def infer_lora_rank(path):
    with safe_open(Path(path), framework="pt", device="cpu") as checkpoint:
        ...
        key = next((name for name in names if name.endswith(".lora_A.weight")), None)
        if key is None:
...
def capture_base_parameter_samples(transformer, sample_count=6, sample_length=2048):
    ...
    if not candidates:
        raise RuntimeError("No non-LoRA transformer parameters were found for the audit.")
    indices = sorted(
        {
            round(index * (len(candidates) - 1) / max(sample_count - 1, 1))
            for index in range(min(sample_count, len(candidates)))
        }
    )
    return {
        candidates[index][0]: candidates[index][1]
        .detach()
        .reshape(-1)[:sample_length]
        .float()
        .cpu()
        .clone()
...
def load_runtime(inference_assets, adapter_specs, dtype="bf16", max_text_length=512):
    inference_model = inference_assets["inference_model"]
    model_configuration = ModelConfig(
        name_or_path=str(Path(inference_model["local_directory"])),
        arch="flux",
        dtype=dtype,
        vae_dtype=dtype,
        te_dtype=dtype,
    ...
    model = StableDiffusion(device="cuda:0", model_config=model_configuration, dtype=dtype)
    model.load_model()
    ...
    if transformer is None:
        raise RuntimeError("The FLUX transformer could not be resolved.")
    transformer.eval()
    ...
    for adapter_spec in adapter_specs:
        ...
        network = LoRASpecialNetwork(
            ...
            is_flux=True,
            is_transformer=bool(getattr(model, "is_transformer", True)),
            base_model=model,
        )
