...
from flux_runtime import load_runtime, sha256_file, unload_runtime
from PIL import Image, ImageDraw
from toolkit.config_modules import GenerateImageConfig

request = json.loads(Path(os.environ["FLUX_EVAL_REQUEST"]).read_text(encoding="utf-8"))
assets = json.loads(Path(request["inference_asset_manifest"]).read_text(encoding="utf-8"))
selection = request["active_checkpoint"]
output_directory = Path(request["output_root"]) / "scale_sweep"
...
for scale in request["scale_sweep"]:
    controller.set_scales({request["adapter_name"]: float(scale)})
    configurations = []
    expected = []
...
metadata = {
    ...
    "scales": [float(scale) for scale in request["scale_sweep"]],
...
print("__FLUX_RESULT__" + json.dumps(metadata))
