...
def _build_provenance(
    ...
    return {
        ...
        "model_format": "diffusers_modular_pipeline",
...
def create_evaluation_bundle(
    ...
    try:
        ...
        if include_manifests:
            ...
            yaml_source = paths.run_dir(run_name) / "train_flux_lora.yaml"
            if yaml_source.is_file():
                config_dir = staging / "config"
                config_dir.mkdir(parents=True, exist_ok=True)
                shutil.copy2(yaml_source, config_dir / "train_flux_lora.yaml")
        else:
