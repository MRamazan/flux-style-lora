...
def default_sample_prompts(trigger_word: str) -> list[str]:
    return [
        f"a serene mountain landscape at golden hour, in {trigger_word} style",
        f"a portrait of an elderly fisherman with a weathered face, in {trigger_word} style",
        f"a quiet city street in the rain at night, in {trigger_word} style",
    ]
...
def build_training_configuration(
    ...
    model_directory = Path(training_model["local_directory"])
    sample_section = {
        "sampler": "flowmatch",
    ...
    process = {
        ...
        "model": {
            "name_or_path": str(model_directory),
            "arch": MODEL_ARCHITECTURE,
            "dtype": config.training_dtype,
            "vae_dtype": config.training_dtype,
            "te_dtype": config.training_dtype,
            "quantize": config.quantize_transformer,
            "quantize_te": config.quantize_text_encoder,
...
def preflight_configuration(
    ...
    if model_section.get("arch") != MODEL_ARCHITECTURE:
        raise TrainingError("The model architecture must be flux.")
    ...
    if not network_section.get("transformer_only"):
        raise TrainingError("FLUX LoRA training must target the transformer only.")
...
def write_configurations(
    ...
    production_path = run_directory / "train_flux_lora.yaml"
    smoke_path = run_directory / "train_flux_lora_smoke.yaml"
...
def run_source_preflight(paths: ProjectPaths, production_path: str) -> dict[str, Any]:
    ...
    environment["FLUX_TRAINING_CONFIGURATION"] = production_path
    ...
    if preflight.get("model_arch") != MODEL_ARCHITECTURE or not preflight.get("is_flux"):
        raise TrainingError(
            "The resolved AI Toolkit model is not a FLUX model. "
            f"Reported arch: {preflight.get('model_arch')}, is_flux: {preflight.get('is_flux')}."
        )
    return preflight
...
def write_run_manifest(
    ...
    manifest = {
        ...
        "model_repository": asset_manifest.get("training_model", {}).get("repository"),
