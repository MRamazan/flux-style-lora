...
from flux_style_lora import evaluation
from flux_style_lora.checkpoints import inventory_from_directory, select_active
from flux_style_lora.configuration import EvaluationConfig
from flux_style_lora.errors import EvaluationError
from flux_style_lora.manifests import read_json, write_json_atomic
from flux_style_lora.paths import ProjectPaths
from flux_style_lora.types import TrainingRun
...
def test_build_evaluation_request_structure(tmp_path, make_lora_checkpoint):
    ...
    assert request["adapter_name"] == "style_adapter"
    assert request["lora_alpha"] == 8
    assert request["output_root"].endswith("style_v1")
...
def test_evaluate_assembles_manifest_with_mocked_scripts(
    ...
    assert paths.evaluation_manifest("style_v1").is_file()
...
def test_report_select_checkpoint_updates_active(tmp_path, monkeypatch, make_lora_checkpoint):
    paths, run = _run(tmp_path, make_lora_checkpoint)
    write_json_atomic(paths.checkpoint_inventory("style_v1"), run.details["checkpoint_inventory"])
    ...
    selection = read_json(paths.active_checkpoint("style_v1"))
