...
from flux_style_lora import bundle
from flux_style_lora.errors import BundleValidationError
...
def test_creates_selected_only_bundle(prepared_run):
    ...
    assert manifest["bundle_type"] == "flux_style_lora_evaluation_bundle"
    assert manifest["bundle_format_version"] == 1
    assert manifest["run_name"] == run_name
    assert manifest["trigger_word"] == "mystyle"
...
def test_rejects_missing_selected_lora(prepared_run):
    paths, run_name = prepared_run()
    from flux_style_lora.manifests import read_json
...
def test_bundle_manifest_records_provenance(prepared_run):
    ...
    assert provenance["training_model"]["repository"] == "black-forest-labs/FLUX.1-dev"
    assert provenance["bundled_vae"] is True
