...
from flux_style_lora.checkpoints import (
...
from flux_style_lora.errors import CheckpointError
...
def test_discover_checkpoint_files_finds_numbered_and_final(tmp_path, make_lora_checkpoint):
    ...
    _populate(directory, "style_v1", [100, 200], make_lora_checkpoint, final=True)
    discovered = discover_checkpoint_files(directory, "style_v1")
...
def test_inspect_checkpoint_infers_rank(tmp_path, make_lora_checkpoint):
    path = make_lora_checkpoint(tmp_path / "style_v1_000100.safetensors", rank=16)
...
def test_inventory_marks_completed_when_configured_step_reached(tmp_path, make_lora_checkpoint):
    directory = tmp_path / "run"
    _populate(directory, "style_v1", [100, 200], make_lora_checkpoint)
    inventory = inventory_from_directory(
        directory, "style_v1", configured_steps=200, source_kind="production", process_status={}
    )
...
def test_inventory_interrupted_run_is_incomplete_and_excludes_final(tmp_path, make_lora_checkpoint):
    directory = tmp_path / "run"
    _populate(directory, "style_v1", [100], make_lora_checkpoint, final=True)
    inventory = inventory_from_directory(
        directory,
        "style_v1",
...
def test_inventory_schema_mismatch_raises(tmp_path, make_lora_checkpoint):
    directory = tmp_path / "run"
    make_lora_checkpoint(directory / "style_v1_000100.safetensors", rank=4)
    make_lora_checkpoint(directory / "style_v1_000200.safetensors", rank=4, extra_key=True)
    with pytest.raises(CheckpointError):
        inventory_from_directory(
            directory,
            "style_v1",
...
def test_select_sweep_modes(tmp_path, make_lora_checkpoint):
    directory = tmp_path / "run"
    _populate(directory, "style_v1", [100, 200, 300, 400], make_lora_checkpoint)
    inventory = inventory_from_directory(
        directory, "style_v1", configured_steps=400, source_kind="production", process_status={}
    )
