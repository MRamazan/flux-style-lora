...
EXCLUDED = {ROOT / "src" / "flux_style_lora" / "secrets.py"}
...
def test_authentication_module_is_removed() -> None:
    assert not (ROOT / "src" / "flux_style_lora" / "authentication.py").exists()


def test_downloads_pass_token_none() -> None:
    assets_source = (ROOT / "src" / "flux_style_lora" / "assets.py").read_text(encoding="utf-8")
    assert assets_source.count("token=None") >= 2
