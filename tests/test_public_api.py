...
from flux_style_lora import ImportedRun, StyleLoraPipeline, TrainingRun
...
def test_pipeline_has_required_public_operations() -> None:
    ...
    assert required <= set(dir(StyleLoraPipeline))


def test_import_evaluation_bundle_takes_single_zip_path() -> None:
    signature = inspect.signature(StyleLoraPipeline.import_evaluation_bundle)
...
def test_evaluation_asset_preparation_takes_no_token() -> None:
    signature = inspect.signature(StyleLoraPipeline.prepare_evaluation_assets)
